"""
run_quant_fixed.py  [DEPRECATED — use scripts/run_quantification.py]
=====================================================================
DEPRECATED: This script writes quantification.json directly, bypassing
canonical_writer.py and the QuantificationRecord schema. It produces
pre-ERA5 fallback winds (5.5 / 4.5 / 3.5 m/s) and the old ±40% bounds.
It is retained here for reproducibility of results_analysis/
quantification.pre_integration.json only.

Use src/quantification/runner.run_quantification() for all new runs.

DeprecationWarning raised at import time.
...
warnings.warn(
    "run_quant_fixed_v0.py is deprecated. Use scripts/run_quantification.py "
    "which calls src.quantification.runner.run_quantification() instead. "
    "This legacy script writes pre-ERA5 winds and ±40% bounds.",
    DeprecationWarning,
    stacklevel=2,
)
import numpy as np
