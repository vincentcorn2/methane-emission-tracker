...
run_quant_fixed.py
==================
*** DEPRECATED — DO NOT USE FOR NEW QUANTIFICATION RUNS ***

BUG: This script sets PIXEL_SIZE = 10.0 for the IME plume-length calculation
but passes raw 10m arrays directly into run_cemf(), which hardcodes
PIXEL_AREA_20M = 400 m² per pixel. Since 10m pixels have a true area of
100 m², this causes a 4× systematic overestimate of total column mass and
therefore a 4× overestimate of flow rate.

USE INSTEAD: scripts/run_cemf_neurath_belchatow.py
  - Correctly downsamples B11, B12, and mask to 20m before calling run_cemf()
  - Passes pixel_size_m=20 to IMECalculator (default)
  - All quantification records produced here should be considered unreliable

Affected legacy records in quantification.json (NO_SCENE_ID, pre-2026-05-14):
  - groningen   28.76 kg/h    (excluded FP — irrelevant)
  - maasvlakte  426.5 kg/h    (v8 non-detection — irrelevant)
  - lippendorf  1579.63 kg/h  (excluded terrain artifact — irrelevant)
  - belchatow   475.92 kg/h   DELETED 2026-05-14; replaced by scene-keyed records
