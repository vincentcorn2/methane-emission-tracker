...
SITES = [
    ...
    # ── Scale-up confirmed detections (2026-04-10) — BT reference for confirmation
    {
        ...
        "note": "Bełchatów lignite mine — EMITTER DETECTED v8 S/C=27.303. Winter reference needed to confirm via BT and rule out seasonal artifact.",
    },
    {
        "name": "lippendorf",
        "tile_id": "T33UUS",
...
if __name__ == "__main__":
    ...
    for site, result in manifest.items():
        ...
        extra  = ""
        if status == "ok":
            cloud_val = result.get('cloud_cover') or 0
            extra = f"  {result.get('acquisition_date','?')}  cloud={cloud_val:.0f}%"
        elif status == "cached":
