"""Build the portfolio risk dashboard notebook."""
import json
import nbformat
...
cells.append(new_markdown_cell(
    ...
    "---\n"
    "**Model**: CH4Net v2 (13.5M param U-Net, Sentinel-2 L1C input, 100\xd7100 patch tiling)  \n"
    "**Coverage**: 6 sites \xd7 up to 4 acquisition dates (Summer 2024)  \n"
    "**Pipeline**: Copernicus S2 L1C \u2192 CH4Net inference \u2192 S/C ratio \u2192 CFAR detection "
    "\u2192 IME/CEMF quantification \u2192 EU ETS carbon liability  \n"
...
# ── Cell 1: Load data ──────────────────────────────────────────────────────────
cells.append(new_markdown_cell("## 1. Load Validation Data"))
cells.append(new_code_cell(
...
# ── Cell 2: Risk model ─────────────────────────────────────────────────────────
cells.append(new_markdown_cell("## 2. Risk Model \u2014 Per-Site Carbon Liability"))
cells.append(new_code_cell(
    "model = RiskModel()\n"
    "\n"
...
# ── Cell 4: S/C ratio time-series plot ────────────────────────────────────────
cells.append(new_markdown_cell(
    "## 4. S/C Ratio Time Series\n"
    "Each point is one Sentinel-2 acquisition. S/C = site_mean / ctrl_mean.  \n"
    "Detection threshold: **S/C > 1.15** (CFAR, CV-adaptive). "
    "Bad scenes (degenerate model output where site_mean = 0.4257) are marked separately.\n"
))
cells.append(new_code_cell(
...
# ── Cell 5: Carbon liability bar chart ────────────────────────────────────────
cells.append(new_markdown_cell(
...
# ── Cell 6: Portfolio bar chart ────────────────────────────────────────────────
cells.append(new_markdown_cell("## 6. Portfolio Carbon Exposure by Ticker"))
cells.append(new_code_cell(
...
# ── Cell 7: EU site map ────────────────────────────────────────────────────────
cells.append(new_markdown_cell("## 7. EU Emission Site Map"))
cells.append(new_code_cell(
    "SITE_COORDS = {\n"
...
cells.append(new_markdown_cell("## 8. Detection Statistics Summary Table"))
cells.append(new_code_cell(
    "summary_rows = []\n"
...
cells.append(new_markdown_cell(
    ...
    "Key finding: Lippendorf collapses **315 -> 0.195** under BT "
    "(summer SWIR vegetation artifact confirmed).\n"
))
cells.append(new_code_cell(
...
out_path = 'results_analysis/methane_portfolio_risk_dashboard.ipynb'
