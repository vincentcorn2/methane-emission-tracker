...
>
> **What it found.** At KWB Bełchatów (Europe's largest coal mine), 30 above-threshold detections and 26 quantification-supporting observations spanning 2019–2025 yield a detection-weighted, cloud-free-conditioned annual estimate of **16,486 t CH₄/yr** (not a full annual mass balance), translating to an illustrative carbon exposure of **€32.31 M/yr** under EU ETS-equivalent pricing.
>
> **What it cannot find — and why this matters.** At Rybnik — validated by both TROPOMI and Carbon Mapper — the conformal threshold never triggers: the model produces sub-threshold signals (January 2023: 2,596 kg/hr, consistent with Carbon Mapper) but the CFAR gate, inflated by Silesian industrial-fringe terrain heterogeneity, suppresses every acquisition below τ. Underground hard-coal operations in this terrain should be treated as unverifiable by this approach, not as zero-emitting.
...
> KWB Bełchatów (PGE, Poland): **30 detections over 2019–2025**, every one in a month when Climate TRACE independently reports 1,700–3,000 t CH₄ emitted. Detection-weighted, cloud-free-conditioned annual estimate: **16,486 t CH₄/yr** (95% CI: 6,781–26,191 t/yr; not a full annual mass balance). Cross-validated on the strongest date by TROPOMI (+12.7 ppb, same-day). Critically, these detections occur at a site the model was *explicitly trained to treat as a non-emitter* — and a formal temporal leakage audit confirms zero date-overlap between training crops and evaluation acquisitions, so the result is a test of generalisation, not memorisation.
>
...
> Rybnik (Silesia) is confirmed by five TROPOMI enhancements and four Carbon Mapper quantifications at 1,150–2,019 kg/hr — the most externally validated site in this study. The conformal threshold never triggers there: no acquisition clears τ = 3.5796. The cause is a training-distribution gap in complex industrial-fringe terrain that inflates the CFAR background gate above what the signal achieves — not an absence of signal. The January 2023 acquisition produces a sub-threshold quantification of 2,596 kg/hr (sc_cfar = 1.48), broadly consistent with the Carbon Mapper range, yet still suppressed by the elevated terrain background. **Any portfolio methane risk assessment using optical satellite monitoring should treat underground hard-coal operations as currently unverifiable by this method, not as zero-emitting.**
>
...
CH4Net runs on a single image — no reference scene required. This matters for continuous emitters: any "clean" reference at a coal mine also contains methane absorption, so reference-subtraction methods partially cancel the signal they are designed to detect. We confirmed this empirically at Bełchatów — opposite bitemporal outcomes on the two strongest historical dates with identical model, site, and reference scene (Appendix C; technical_appendix.md §2.2).
...
The model is not blind to Rybnik. Sub-threshold signals are present on multiple dates — the January 2023 acquisition yields 2,596 kg/hr (sc_cfar = 1.48) within the Carbon Mapper range — but the CFAR gate, inflated by Silesian industrial-fringe terrain heterogeneity, suppresses them below τ = 3.5796. A designed falsification on March 22, 2025 confirms the signals are not static terrain artifacts: the probability centroid bearing flipped 172° between consecutive dates, tracking a wind direction change that a fixed surface feature cannot reproduce (full centroid-wind geometry in technical_appendix.md §5.3; acquisition tables in Appendix D §3.3).

Critically, the model is not blind to Rybnik — it produces signals, just not above the terrain-inflated CFAR gate. The January 2023 acquisition yields a sub-threshold quantification of 2,596 kg/hr (sc_cfar = 1.48, cfar_detect = False); the August 2023 acquisition yields 71 kg/hr (sc_cfar = 2.39, cfar_detect = False). Neither clears τ = 3.5796. The January figure sits within the Carbon Mapper range for this source (1,150–2,019 kg/hr), suggesting the underlying spectral signal is present but the terrain background is too heterogeneous for the CFAR gate to resolve it above the conformal threshold.

The failure is a training-distribution gap that elevates the effective detection floor, not a fundamental detection limit. CH4Net v8 was fine-tuned primarily on power-station and oil-and-gas terrain. Underground hard-coal venting in complex industrial fringe terrain — Silesian collieries surrounded by urban infrastructure and legacy industry — was under-represented, the same way the original CH4Net was biased toward Turkmenistan desert terrain before domain adaptation. The fix is the same: retrain on confirmed Silesian scenes. Until then, **optical satellite monitoring of underground hard-coal operations in industrial-fringe terrain should be treated as systematically undercounting, not as zero-emission evidence.**
...
The above-threshold record also guards against label-reproduction: Bełchatów was included in v8 training as a negative example, labeled as a non-methane site on the August 2024 background tile. The model nonetheless produces 30 above-threshold responses — it is responding to spectral evidence that contradicts its own training label. A formal temporal leakage audit (Appendix F) confirms zero date-overlap between the August 2024 training tile and any evaluation acquisition, ruling out data leakage as the explanation. This makes the detection record a stronger generalisation argument than a held-out test: the model is overriding a label it was given, not reproducing one it never saw. This is demonstrated at a single site; whether the behaviour holds across a broader set of European emitter types remains an open question.
...
| **Rybnik** | **0 above-threshold** (2 sub-threshold quantifications: Jan 2023 2,596 kg/hr, Aug 2023 71 kg/hr) | 5 enhancements (+10.85–19.66 ppb, 2023–2024) | 6 overpasses; 4 quantified (1,150–2,019 kg/hr, 2023–2026) | N/A — conformal threshold never triggers | **Conformal rule never fires** — CFAR gate inflated by Silesian terrain heterogeneity suppresses sub-threshold signals below τ = 3.5796 |
...
- **At Europe's largest coal mine, we found a multi-year emission signal.** KWB Bełchatów produced 30 confirmed detections over 2019–2025. The detection-weighted, cloud-free-conditioned annual estimate — 16,486 t CH₄/yr (not a full annual mass balance) — is consistent with an independent inventory and was confirmed on its strongest date by a second, entirely different satellite sensor (TROPOMI).
...
Coal-mine methane is not currently covered under the EU ETS (Phase IV), but the Methane Regulation sets the trajectory. The figures below represent order-of-magnitude exposure if a comparable price regime applied. Converting the pipeline's cloud-free-conditioned central estimate (16,486 t CH₄/yr; detection-weighted, not a full annual mass balance) via IPCC AR5 GWP100 (factor 28) gives 461,608 t CO₂e/yr. Sensitivity across EUA price scenarios:
...
| Source | Relative σ | Variance share (σᵢ²/Σσᵢ²) |
|---|---:|---:|
| Carbon price trajectory (log-vol 30%) | 0.300 | **47.5%** |
| Emission sampling (26-obs CI) | 0.286 | 43.2% |
| ERA5 wind bias (±10%) | 0.100 | 5.3% |
| Plume spatial extent (±15%) | 0.087 | 4.0% |
| **Combined (quadrature)** | **0.435** | **100%** |

Carbon price and emission sampling are the two dominant sources, contributing roughly equally (47.5% and 43.2%). Satellite measurement uncertainties — ERA5 wind bias and plume spatial extent — together account for only ~9% of total variance. The practical implication: improving emission measurement precision would reduce the second row but leave the dominant sources of uncertainty largely untouched. As a robustness check: even if the true annual emission rate were 2× our central estimate (32,972 t/yr), satellite measurement uncertainties would still account for only ~9% of total variance — the liability distribution widens but its shape is governed by regulatory and sampling uncertainty, not by sensor precision. **The wide tail in Climate VaR reflects whether and when a comparable price regime applies, and how well the 26-observation sample represents annual emissions — not a fundamental measurement limit of the satellite.**
...
Bitemporal differencing is disabled at coal mine sites for the reason detailed in §3.1 and Appendix C: at a continuously emitting facility any reference scene itself contains methane absorption, making the BT direction uninterpretable. The empirical confirmation (opposite outcomes on the two strongest Bełchatów dates, same model and reference) is in Appendix C.
