...
The Bełchatów tile contains two large industrial assets roughly 3 kilometres apart: the KWB open-pit lignite mine and the Bełchatów power station that burns its output. Early site analysis, like most initial work on Central European industrial methane, centred on the power station — the dominant visible feature in the tile and the logical starting point before facility-level inventory data was available. It was only after integrating Climate TRACE and Carbon Mapper asset-level data mid-project that the distinction became operationally important: the power station is classified under IPCC sector 1A1a (electricity generation) and carries near-zero reported methane, while the adjacent mine is classified under sector 1B1a (fugitive emissions from coal mining) and is the methane-dominant asset. We re-ran the full pipeline on the correct mine coordinates with an OSM polygon boundary.

Two things then happened that independently confirm the shift was correct.

**Physics validation.** Applying the upgraded MBSP retrieval (Varon 2021, scene-derived band-scaling factor *c* — see §6.4) to the old 5 km power-station crop produced a −95% collapse in the mean estimated flow rate (6,739 → 320 kg/hr). The old heuristic had been inflating results over a large, spectrally heterogeneous surface that included roads, agricultural fields, and industrial infrastructure well outside the mine boundary. The correct mine polygon — a smaller, spectrally coherent boundary — yields estimates physically consistent with external inventory at the 56% single-pass recovery ratio expected from Sentinel-2 sensing.

**Detection continuity.** When the crop shifted to the mine, the model continued to produce above-threshold S/C responses. The detection record did not disappear — it strengthened and aligned more cleanly with Climate TRACE monthly inventory. This is consistent with the model having been responding to the mine's methane signal all along, even when the crop boundary extended well beyond it.

The two-stage comparison — power station then coal mine — turns out to add something useful to the analysis. It demonstrates that the MBSP retrieval is sensitive enough to discriminate between a real signal and surface-driven inflation: the right crop produces physically coherent results; the wrong one inflates by nearly an order of magnitude. The archived crop-comparison data (all five variants, old and new, with and without MBSP) is available at `results_analysis/timeseries/belchatow/` for full reproducibility.
...
The record of above-threshold responses also provides evidence against a label-reproduction artifact. Bełchatów was included in the v8 training data as a negative example: we labeled it as a non-methane site on the background tile from August 2024. The model nonetheless produces 30 above-threshold responses — the pipeline is responding to spectral signatures it was trained to ignore. This is consistent with the model having learned a methane-specific spectral signature rather than memorizing site identities — and it is a stronger indicator than a held-out test would provide, because the model is disagreeing with its own training label on the basis of the spectral evidence. We note this is demonstrated at a single site; whether the same behaviour holds across a broader set of European emitter types remains an open question for future work.
